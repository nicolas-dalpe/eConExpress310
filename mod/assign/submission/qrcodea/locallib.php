...
define('ASSIGNSUBMISSION_QRCODEA_MAXSUMMARYFILES', 5);
define('ASSIGNSUBMISSION_QRCODEA_FILEAREA', 'submission_qrcodea');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assign_submission_qrcodea extends assign_submission_plugin {
    ...
    public function get_name() {
        return get_string('qrcodea', 'assignsubmission_qrcodea');
    }
    ...
    private function get_file_submission($submissionid) {
        global $DB;
        return $DB->get_record('assignsubmission_qrcodea', array('submission'=>$submissionid));
    }
    ...
    public function get_settings(MoodleQuickForm $mform) {
        ...
        if ($this->assignment->has_instance()) {
            ...
            $defaultmaxfilesubmissions = get_config('assignsubmission_qrcodea', 'maxfiles');
            $defaultmaxsubmissionsizebytes = get_config('assignsubmission_qrcodea', 'maxbytes');
            $defaultfiletypes = get_config('assignsubmission_qrcodea', 'filetypes');
        }
        $defaultfiletypes = (string)$defaultfiletypes;

        $settings = array();
        $options = array();
        for ($i = 1; $i <= get_config('assignsubmission_qrcodea', 'maxfiles'); $i++) {
            $options[$i] = $i;
        }

        $name = get_string('maxfilessubmission', 'assignsubmission_qrcodea');
        $mform->addElement('select', 'assignsubmission_qrcodea_maxfiles', $name, $options);
        $mform->addHelpButton('assignsubmission_qrcodea_maxfiles',
                              'maxfilessubmission',
                              'assignsubmission_qrcodea');
        $mform->setDefault('assignsubmission_qrcodea_maxfiles', $defaultmaxfilesubmissions);
        $mform->hideIf('assignsubmission_qrcodea_maxfiles', 'assignsubmission_qrcodea_enabled', 'notchecked');

        $choices = get_max_upload_sizes($CFG->maxbytes,
                                        $COURSE->maxbytes,
                                        get_config('assignsubmission_qrcodea', 'maxbytes'));

        $settings[] = array('type' => 'select',
                            'name' => 'maxsubmissionsizebytes',
                            'description' => get_string('maximumsubmissionsize', 'assignsubmission_qrcodea'),
                            'options'=> $choices,
                            'default'=> $defaultmaxsubmissionsizebytes);

        $name = get_string('maximumsubmissionsize', 'assignsubmission_qrcodea');
        $mform->addElement('select', 'assignsubmission_qrcodea_maxsizebytes', $name, $choices);
        $mform->addHelpButton('assignsubmission_qrcodea_maxsizebytes',
                              'maximumsubmissionsize',
                              'assignsubmission_qrcodea');
        $mform->setDefault('assignsubmission_qrcodea_maxsizebytes', $defaultmaxsubmissionsizebytes);
        $mform->hideIf('assignsubmission_qrcodea_maxsizebytes',
                           'assignsubmission_qrcodea_enabled',
                           'notchecked');

        $name = get_string('acceptedfiletypes', 'assignsubmission_qrcodea');
        $mform->addElement('filetypes', 'assignsubmission_qrcodea_filetypes', $name);
        $mform->addHelpButton('assignsubmission_qrcodea_filetypes', 'acceptedfiletypes', 'assignsubmission_qrcodea');
        $mform->setDefault('assignsubmission_qrcodea_filetypes', $defaultfiletypes);
        $mform->hideIf('assignsubmission_qrcodea_filetypes', 'assignsubmission_qrcodea_enabled', 'notchecked');
    }
    ...
    public function save_settings(stdClass $data) {
        $this->set_config('maxfilesubmissions', $data->assignsubmission_qrcodea_maxfiles);
        $this->set_config('maxsubmissionsizebytes', $data->assignsubmission_qrcodea_maxsizebytes);

        if (!empty($data->assignsubmission_qrcodea_filetypes)) {
            $this->set_config('filetypeslist', $data->assignsubmission_qrcodea_filetypes);
        } else {
    ...
    private function get_file_options() {
        ...
        if ($fileoptions['maxbytes'] == 0) {
            // Use module default.
            $fileoptions['maxbytes'] = get_config('assignsubmission_qrcodea', 'maxbytes');
        }
        return $fileoptions;
    }
    ...
    public function get_form_elements($submission, MoodleQuickForm $mform, stdClass $data) {
        ...
        $data = file_prepare_standard_filemanager($data,
                                                  ...
                                                  'assignsubmission_qrcodea',
                                                  ASSIGNSUBMISSION_QRCODEA_FILEAREA,
                                                  $submissionid);
    ...
    public function save(stdClass $submission, stdClass $data) {
        ...
        $count = $this->count_files($submission->id, ASSIGNSUBMISSION_QRCODEA_FILEAREA);
        ...
        $event = \assignsubmission_qrcodea\event\assessable_uploaded::create($params);
        ...
        if ($filesubmission) {
            $filesubmission->numfiles = $this->count_files($submission->id,
                                                           ASSIGNSUBMISSION_QRCODEA_FILEAREA);
            $updatestatus = $DB->update_record('assignsubmission_qrcodea', $filesubmission);
            $params['objectid'] = $filesubmission->id;

            $event = \assignsubmission_qrcodea\event\submission_updated::create($params);
            ...
            $filesubmission->id = $DB->insert_record('assignsubmission_qrcodea', $filesubmission);
            $params['objectid'] = $filesubmission->id;

            $event = \assignsubmission_qrcodea\event\submission_created::create($params);
            $event->set_assign($this->assignment);
            $event->trigger();
    ...
    public function remove(stdClass $submission) {
        ...
        if ($currentsubmission) {
            ...
            $DB->update_record('assignsubmission_qrcodea', $currentsubmission);
        }
    ...
    public function view_summary(stdClass $submission, & $showviewlink) {
        ...
        $showviewlink = $count > ASSIGNSUBMISSION_QRCODEA_MAXSUMMARYFILES;
        if ($count <= ASSIGNSUBMISSION_QRCODEA_MAXSUMMARYFILES) {
            return $this->assignment->render_area_files('assignsubmission_qrcodea',
                                                        ASSIGNSUBMISSION_QRCODEA_FILEAREA,
                                                        $submission->id);
        } else {
            return get_string('countfiles', 'assignsubmission_qrcodea', $count);
        }
    }
     ...
     *
     * In our case it allows us to inject the QR Code graphic right after the
     * description.
     *
     * @return string
     */
    ...
    public function upgrade(context $oldcontext,
        ...
        if (!$DB->insert_record('assignsubmission_qrcodea', $filesubmission) > 0) {
    ...
    public function delete_instance() {
        ...
        $DB->delete_records('assignsubmission_qrcodea',
    ...
    public function format_for_log(stdClass $submission) {
        ...
        $filecount = $this->count_files($submission->id, ASSIGNSUBMISSION_QRCODEA_FILEAREA);

        return get_string('numfilesforlog', 'assignsubmission_qrcodea', $filecount);
    }
    ...
    public function is_empty(stdClass $submission) {
        return $this->count_files($submission->id, ASSIGNSUBMISSION_QRCODEA_FILEAREA) == 0;
    }
    ...
    public function get_file_areas() {
        return array(ASSIGNSUBMISSION_QRCODEA_FILEAREA=>$this->get_name());
    }
    ...
    public function copy_submission(stdClass $sourcesubmission, stdClass $destsubmission) {
        ...
        // Copy the assignsubmission_qrcodea record.
        if ($filesubmission = $this->get_file_submission($sourcesubmission->id)) {
            unset($filesubmission->id);
            $filesubmission->submission = $destsubmission->id;
            $DB->insert_record('assignsubmission_qrcodea', $filesubmission);
        }
        return true;
    }
