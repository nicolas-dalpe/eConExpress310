...
/**
 * Test helpers for the hybrid question type.
 *
...
/**
 * Test helper class for the hybrid question type.
 *
 * @copyright  2013 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_hybrid_test_helper extends question_test_helper {
     ...
     * @return qtype_hybrid_question
     */
    protected function initialise_hybrid_question() {
        question_bank::load_question_definition_classes('hybrid');
        $q = new qtype_hybrid_question();
        test_question_maker::initialise_a_question($q);
        $q->name = 'Hybrid question (HTML editor)';
        ...
        $q->qtype = question_bank::get_qtype('hybrid');

        return $q;
    }

    /**
     * Makes an hybrid question using the HTML editor as input.
     * @return qtype_hybrid_question
     */
    public function make_hybrid_question_editor() {
        return $this->initialise_hybrid_question();
    }

    /**
     * Make the data what would be received from the editing form for an hybrid
     ...
     */
    public function get_hybrid_question_form_data_editor() {
        $fromform = new stdClass();

        $fromform->name = 'Hybrid question (HTML editor)';
    ...
    /**
     * Makes an hybrid question using the HTML editor allowing embedded files as
     * input, and up to three attachments.
     * @return qtype_hybrid_question
     */
    public function make_hybrid_question_editorfilepicker() {
        $q = $this->initialise_hybrid_question();
     ...
     * @return qtype_hybrid_question
     */
    public function make_hybrid_question_editorfilepickertworequired() {
        $q = $this->initialise_hybrid_question();
        $q->responseformat = 'editorfilepicker';
        $q->attachments = 2;
     ...
     * @return stdClass the data that would be returned by $form->get_gata();
     */
    public function get_hybrid_question_form_data_editorfilepicker() {
        $fromform = new stdClass();

        $fromform->name = 'Hybrid question with filepicker and attachments';
        $fromform->questiontext = array('text' => 'Please write a story about a frog.', 'format' => FORMAT_HTML);
        $fromform->defaultmark = 1.0;
        $fromform->generalfeedback = array('text' => 'I hope your story had a beginning, a middle and an end.', 'format' => FORMAT_HTML);
    ...
    /**
     * Makes an hybrid question using plain text input.
     * @return qtype_hybrid_question
     */
    public function make_hybrid_question_plain() {
        $q = $this->initialise_hybrid_question();
     ...
     * @return stdClass the data that would be returned by $form->get_gata();
     */
    public function get_hybrid_question_form_data_plain() {
        $fromform = new stdClass();
    ...
    /**
     * Makes an hybrid question using monospaced input.
     * @return qtype_hybrid_question
     */
    public function make_hybrid_question_monospaced() {
        $q = $this->initialise_hybrid_question();
        $q->responseformat = 'monospaced';
        return $q;
    }

    public function make_hybrid_question_responsetemplate() {
        $q = $this->initialise_hybrid_question();
    ...
    /**
     * Makes an hybrid question without an online text editor.
     * @return qtype_hybrid_question
     */
    public function make_hybrid_question_noinline() {
        $q = $this->initialise_hybrid_question();
