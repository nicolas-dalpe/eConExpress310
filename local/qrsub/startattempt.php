...
if ($currentattemptid) {
    if ($lastattempt->state == quiz_attempt::OVERDUE) {
        ...
        // $attemptobj = quiz_create_attempt_handling_errors($currentattemptid, $id);
        $attemptobj = quiz_attempt::create($currentattemptid);
        list($slots, $page) = qrsub::get_first_hybrid_question($attemptobj, $page);

        $url = str_replace('mod/quiz', 'local/qrsub', $quizobj->attempt_url($currentattemptid, $page));
        redirect($url);
...
// Redirect here on new attempt which shouldn't happen but just in case.
$url = str_replace('mod/quiz', 'local/qrsub', $quizobj->attempt_url($attempt->id, $page));
redirect($url);
